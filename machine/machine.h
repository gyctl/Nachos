...
#define MemorySize 	(NumPhysPages * PageSize)   //主存大小，由物理页面数与页面大小计算可得
...
// the contents of the TLB are free to be modified by the kernel software.
  ...
  TranslationEntry *tlb;		// this pointer should be considered 
	...
	TranslationEntry *pageTableForTlb;    //为了使用tlb设置的系统页表，大小等于用户页面大小
	TranslationEntry *pageTableForAll;      //为了使用多线程设置的倒排页表，大小为物理页面个数
	// unsigned int pageTableForAllSize;


	float tlb_hit;
  float tlb_miss;

	int bitmap[NumPhysPages];     //CHANGE
	int AllocatePage();         //allocate  page for user program
 	void RecyclePage();  //recycle page from user program
	 void PCAdvance();
    ...
    unsigned int pageTableSize;
		unsigned int pageTableForTlbSize;
