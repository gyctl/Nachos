...
// from the virtual address
    ...
    for (int i=0 ; i<NumPhysPages; i++){    //find all of the mainmemory
			if (pageTableForAll[i].valid == true && pageTableForAll[i].virtualPage == vpn 
			&& pageTableForAll[i].pid == currentThread->getId()){
				// printf("=====命中=========\n");
				pageTableForAll[i].lru++;
				for (int j=0;j<NumPhysPages;j++){
					if (j==i) continue;
					pageTableForAll[i].lru/=2;
				}
				entry = &pageTableForAll[i];
				break;
			} else {
				// printf("=========findvpn%d======\n",vpn);
			}
